...
$test = get-vm|Where-Object {$_.name -like '*' + $VMname + '*'}
    for($i=0 ; $i -le $test.length -1 ;$i++){
        write-host($i+1)($test.name[$i]) 'State:'($test.state[$i])}
        while(1 -eq 1){
            ...
            [int]$chosen_vm =(Read-host('what vm do you whish to change?'))
                ...
                else{ return $test.name[$chosen_vm]}
                                                }
}
...
while (1 -eq 1){
   $action = Read-Host('please choose an action:`
    ...
    elseif($action -eq '3'){
        ...
        Write-Output('you must enter a correct number')}
